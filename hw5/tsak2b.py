...
if sweets > 0:
    while sweets != 0:
        ...
        if sweets > 0:
            if bot == 1:
# мозг начало
                if sweets >= (max_sweets * 2 + 1):
                    bot1 = max_sweets #сколько бот берет конфет
                    # bot1 = random.randint(max_sweets) # бот должен брать макс конфет кажд раз
                    ...
                    bot1 = max_sweets
                    print(f' bot взял {bot1} конфет')
                    sweets = sweets - bot1  # забираем конфеты, пишем остатокк

                elif sweets <= (max_sweets * 2):
                    # bot2 = max_sweets // 2 - 1
                    # bot1 = random.randint(1, bot2)
                    bot2 = sweets - max_sweets - 1
                    print(f' bot взял {bot2} конфет')
                    sweets = sweets - bot2  # забираем конфеты, пишем остатокк
# мозг конец
            else:
